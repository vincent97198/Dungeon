...
void Dungeon::build_pick_item_Map(map<int, string> &Pick_Item_Map) {
    int Pick_Item_Index = 0;

    for (const auto &iter:Dungeon::player.getCurrentRoom()->getObjects())
        if (check_type::isItemType(iter) != nullptr)
            Pick_Item_Map[Pick_Item_Index++] = iter->getName();
}

void Dungeon::Pick_item(Player player) {
    map<int, string> Pick_Item_Map;
    build_pick_item_Map(Pick_Item_Map);

    vector<int> type;
    Ask_multi_int("Which item do you want to pick?", Pick_Item_Map, type);
